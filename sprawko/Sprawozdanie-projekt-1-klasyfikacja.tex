...
W celu zliczania wystąpień nazw budowli dla poszczególnych krajów, wykorzystano Term Frequency Matrix (TFM). Metoda ta bazuje na znanym zestawie słów kluczowych (słownikach \cite{slowniki}), na podstwaie wzoru \((10)\) określa czy dany, słownikowy wyraz,
wystąpił w tekście, a następnie w wzorze \((9)\) zliczamy wystąpienia dla całości dokumentów.
  ...
  \begin{equation}
    ...
    \end{cases}
  \end{equation}
gdzie $k \in [1,6]$, zaś \(p\) oznacza zbiór słów w danym słwoniku, \(t(i)\) określa liczbę wystąpień w badanym dokumencie \(i\)-tego słowa ze słwonika, \(d\) - zawartość dokumentu. \\
...
% Zakresy miar, co oznaczają ich wartości, przykładowe wartości dla wektorów cech
% 
% UML i opis modułów aplikacji (4.1)
%
...
Po zakończeniu klasyfikacji wyniki wraz z obliczonymi miarami jakości klasyfikacji zostają zapisane do pliku tekstowego o wybranej przez użytkownika nazwie bądź domyślnie o nazwie: \textbf{"results.txt"}
