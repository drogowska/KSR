...
%out - parametry, 
Aplikacja komunikuje sie z użytkownikiem za pomocą konsolowego interfejsu. Aplikacja jest uruchamiana z linii poleceń, wymagane jest posiadanie JDK w wersji 17. 
Aby uruchomić program, należy wygenerować, na podstawie kodu źródłowego, plik jar, a następnie wykonać z lini poleceń polecenie 
\begin{center}
  \textit{java -jar code.jar splitPercent k metric}.
\end{center}
Przy czym aplikacja przyjmuje trzy parametry wejściowe: \(k\) - liczba naturalna, stała określająca ilość sąsiadów branych pod uwagę w algorytmie \(k\)-NN; 
\(splitPercent\) - liczba dodatnia całkowita, z przedziału [1-99], określająca procent zbioru traktowany jako treningowy; 
\(metric\) - do wyboru jedna z: M1 (metryka euklidesowa), M2 (metryka uliczna), M3 (metryka Czebyszewa).
Po zakończeniu klasyfikacji wyniki wraz z obliczonymi miarami jakości klasyfikacji zostają wyświetlone w konsoli, w foramcie tekstowym.

Przykładowe uruchomienie programu dla \(k\) równego 20, podziału 30:70 (treningowy:testowy) i użycie metryki ulicznej:
\begin{center}
  java -jar code.jar 30 20 M2
\end{center}
