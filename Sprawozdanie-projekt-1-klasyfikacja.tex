...
\\
\begin{center}
  PPV$_c = (\sum_{x} PPV$$_x$ $ \cdot n_{x}) / \sum_{x} n_{x}$,\\ 
\end{center}
...
\\
\begin{center}
    TPR$_c = (\sum_{x} PPV$$_x$ $ \cdot n_{x}) / \sum_{x} n_{x}$, \\
  \end{center}
gdzie $n$ jest liczebnością danej klasy, $n\in C$.\\
...
\item miara F1$_c$, określana dla całości wyników klasyfikacji, obliczana jako średnia ważona wartości miary F1 dla każdej z klas. Miara ta przyjmuej wartości z zakresu analogicznego jak w przydapku miary F1. \\
\begin{center}
