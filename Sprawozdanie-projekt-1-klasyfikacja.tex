...
\begin{enumerate}
\item miara dokładności ACC (ang. accuracy) określana dla całości wyników klasyfikacji (liczba dokumentów zaklasyfikowanych poprawnie podzielona przez liczbę wszystkich dokumentów klasyfikowanych)
\break $ACC = \sum_{x} P$$_x$ $_x$ $ / (\sum_{x} P$$_x$ $_x$ $ + \sum_{xy}  F$$_x$ $_y$ $)$, gdzie $ x\neq y$
\item miara precyzji PPV (ang. precision) określana dla poszczególnych klas krajów, do których możemy zaklasyfikować dokumenty (liczba dokumentów zaklasyfikowana poprawnie do danego kraju "x" podzielona przez sumę liczby dokumentów zaklasyfikowanych poprawnie do kraju "x" oraz liczby dokumentów niepoprawnie zaklasyfikowanych do kraju "x")
\break $PPV(x) = P$$_x$ $_x$ $ / (P$$_x$ $_x$ $ + \sum_{y}  F$$_y$ $_x$ $)$ , gdzie $ x\neq y$
\item miara czułości TPR (ang. recall) określana dla poszczególnych klas krajów, do których możemy zaklasyfikować dokumenty (liczba dokumentów zaklasyfikowana poprawnie do danego kraju "x" podzielona przez sumę liczby dokumentów zaklasyfikowanych poprawnie do kraju "x" oraz liczby dokumentów niepoprawnie zaklasyfikowanych do kraju innego niż "x")
\break $TPR(x) = P$$_x$ $_x$ $ / (P$$_x$ $_x$ $ + \sum_{y}  F$$_x$ $_y$ $)$ , gdzie $ x\neq y$
\item miara precyzji PPV$_c$ określana dla całości wyników klasyfikacji (średnia 
\item miara czułości TPR$_c$ określana dla całości wyników klasyfikacji (średnia 
\item miara F1 określana dla całości wyników klasyfikacji (średnia 

% BRAKUJE TUTAJ JESZCZE: 
% - OPISÓW, WZORÓW DLA MIAR PPV, TPR, F1
% - WARTOŚCI KTÓRE MOŻE PRZYJMOWAĆ KAŻDA Z MIAR, I JAK JEST LEPIEJ (WIEKSZA WARTOŚĆ CZY MNIEJSZA?)
% - SPRAWDZENIA POPRAWNOŚCI WZORÓW i OPISÓW
